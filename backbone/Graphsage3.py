...
class AGNN(torch.nn.Module):
    def __init__(self):
        ...
        # 图滤波层，使用GCN
        self.sage_1 = SAGEConv(161, 100)
        self.sage_2 = SAGEConv(100, 50)
    ...
    def forward(self, data, r, h, c):
        ...
        x = self.sage_1(x, edge_index)# x[batch,T,stock,feature]
        x = torch.tanh(x)
        x = self.sage_2(x, edge_index)# x[batch,T,stock,feature]
        x = torch.tanh(x)
...
for epoch in range(num_epoch):
    ...
    for i,(x,r) in enumerate(train_loader):
        ...
        #         torch.save(w, "results/GNNs/Graphsage/train_w.pth")
        #         torch.save(R, "results/GNNs/Graphsage/train_R.pth")
        #         # torch.save(x, "results/GNNs/Graphsage/train_H.pth")
        #         torch.save(mu, "results/GNNs/Graphsage/train_mu.pth")
        #         # torch.save(temp, "results/GNNs/Graphsage/train_sigma的逆.pth")
        #         torch.save(r, "results/GNNs/Graphsage/train_r.pth")
        optimizer.zero_grad()
    ...
    for i,(x,r) in enumerate(valid_loader):
        ...
        #         torch.save(w, "results/GNNs/Graphsage/valid_w.pth")
        #         torch.save(R, "results/GNNs/Graphsage/valid_R.pth")
        #         # torch.save(x, "results/GNNs/Graphsage/valid_H.pth")
        #         torch.save(mu, "results/GNNs/Graphsage/valid_mu.pth")
        #         # torch.save(temp, "results/GNNs/Graphsage/valid_sigma的逆.pth")
        #         torch.save(r, "results/GNNs/Graphsage/valid_r.pth")
        mse = torch.sum(torch.square(r - pre_r)) / (2912 * r_l * batch_size)
    ...
    # # 保存合并后的张量为文件
    # torch.save(combined_tensor, 'results/GNNs/Cheb/R_VAILD.pt')
...
for i,(x,r) in enumerate(test_loader):
    ...
    with torch.no_grad():
        ...
        R,w,x,mu,temp,R_beta = model_agnn(Gdata, r, h, c)#b,t,n
    r_mean = torch.mean(r,dim=1)
    mu = mu.squeeze(-1)
    mu_arg = torch.argsort(mu,dim=1,descending=True).float()
    r_arg = torch.argsort(r_mean,dim=1,descending=True).float()
    loss_order = criterion(mu_arg,r_arg)/2912/batch_size
    optimizer.zero_grad()
    SR = (torch.mean(R, dim=1))/(torch.std(R, dim=1)+1e-8)#b
    # SR2 = torch.pow(SR, 2)
    loss0 = torch.exp(-torch.mean(SR))
    loss1 = loss_1(R_beta)/batch_size
    loss = loss0 + loss1 + 0.1*loss_order
    # print('  Test 夏普比率：{:.4f}'.format(SR.item()),'  Test Loss：{:.6f}'.format(loss.item()))
    test_SRs.append(torch.mean(SR).item())
    test_losses.append(loss.item())
    test_losses1.append(loss0.item())
    test_losses2.append(loss1.item())
    ...
    if i == len(test_loader)-1:
        # torch.save(w, "results/GNNs/Graphsage/test_w.pth")
        # torch.save(R, "results/GNNs/Graphsage/test_R.pth")
        # torch.save(x, "results/GNNs/Graphsage/test_H.pth")
        #torch.save(mu, "results/GNNs/Graphsage/test_mu.pth")
        # torch.save(temp, "results/GNNs/Graphsage/test_sigma的逆.pth")
        torch.save(r, "/home/huquan/industrychain/lhl/backbone_result/G/3/test_r.pth")
        torch.save(pre_r,"/home/huquan/industrychain/lhl/backbone_result/G/3/test_pre_r.pth")
...
#     # 保存合并后的张量为文件
# # print(combined_tensor)
# torch.save(combined_tensor1, 'results/GNNs/Graphsage/R_test.pt')
# torch.save(combined_tensor2, 'results/GNNs/Graphsage/stdR_test.pt')

test_sr = np.average(test_SRs)
test_loss = np.average(test_losses)
test_loss1 = np.average(test_losses1)
test_loss2 = np.average(test_losses2)
test_loss3 = np.average(test_losses3)
print('Test 平均夏普比率：{:.4f}'.format(test_sr),' Test Loss：{:.6f}'.format(test_loss),' Test Loss1：{:.6f}'.format(test_loss1),' Test Loss2：{:.6f}'.format(test_loss2),' Test Loss3：{:.6f}'.format(test_loss3))